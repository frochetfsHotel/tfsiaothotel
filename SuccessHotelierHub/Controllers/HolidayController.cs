...
namespace SuccessHotelierHub.Controllers
{
    ...
    public class HolidayController : Controller
    {
        ...
        public ActionResult Create(HolidayVM model)
        {
            try
            {
                ...
                holidayId = holidayRepository.AddHoliday(model);
                ...
                else
                {
                    ...
                    {
                        IsSuccess = false,
                        errorMessage = "Holiday details not saved successfully."
                    }, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception e)
        ...
        public ActionResult Delete(Guid id)
        {
            try
            {
                ...
                holidayId = holidayRepository.DeleteHoliday(id, LogInManager.LoggedInUserId);
                ...
                else
                {
                    ...
                    {
                        IsSuccess = false,
                        errorMessage = "Holiday details not deleted successfully."
                    }, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception e)
        ...
        [HttpPost]
        public ActionResult Search(SearchHolidayParametersVM model)
        {
            try
            {
                object sortColumn = "";
                ...
                var holidays = holidayRepository.SearchHoliday(model, Convert.ToString(sortColumn), Convert.ToString(sortDirection));

                int totalRecords = 0;
                var dbRecords = holidays.Select(m => m.TotalCount).FirstOrDefault();
